...
from pipelines.ti_custom.custom_dataset import CustomDataset


class TICustom:
    def __init__(self):
        self._model_id = 'stabilityai/stable-diffusion-3.5-large'
        self._embeds_weight_name = 'custom_ti_embeds.safetensors'
    ...
    def learn_embedding_ti(
        ...
        for tokenizer, text_encoder in zip(tokenizers, text_encoders):
            ...
            token_embeds[placeholder_id] = token_embeds[initializer_id].clone()
        ...
        try:
            optimizer = bnb.optim.AdamW8bit(ti_params, lr=ti_learning_rate, weight_decay=0.0)
        except Exception:
            optimizer = torch.optim.AdamW(ti_params, lr=ti_learning_rate)
            print('[WARN] Cannot apply AdamW 8-bit: falling back to standard AdamW', weight_decay=0.0)
